...
basedir <- "C:/Users/memauritz/OneDrive - University of Texas at El Paso/Bahada/CR3000/L1"
...
These graphs show most recent incoming eddy covariance and ancillary data from the tower at the Jornada Bahada Site. Data are summarised for selected timeframe.
...
-------------------------------------------------------------
```{r, echo = FALSE, warning = FALSE, message = FALSE}
# calculate cumulative rainfall, mean/max/min temp
renderTable({
    c <- flux[date >= input$daterange4[[1]] & date <= input$daterange4[[2]],.SD,.SDcols=c("date","date_time","precip_Tot","t_hmp_mean")]
  
c_summary <- c %>%
  summarise(Total_Rainfall_mm = sum(precip_Tot, na.rm=TRUE),
            AirTemp_max_C = max(t_hmp_mean, na.rm=TRUE),
            AirTemp_mean_C = mean(t_hmp_mean, na.rm=TRUE),
            AirTemp_min_C = min(t_hmp_mean, na.rm=TRUE))
})
